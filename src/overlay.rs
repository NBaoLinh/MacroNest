...
mod windows_overlay {
    ...
    fn run_image_search_follow_loop(
        ...
        while image_search_following_is_active(preset.id) {
            ...
            match run_vision_once_with_options(&preset, true, false, variable_override.as_deref(), None, None) {
            ...
            IfConditionType::VisionMatch => {
                ...
                if let Some(pid) = vision_preset_id {
                    ...
                    if let Some(preset) = preset {
                        ...
                        if let Ok(outcome) = run_vision_once_with_options(&preset, false, false, None, None, None) {
            ...
            if let Some(timer) = timer_preset {
                ...
                if timer.is_countdown {
                    ...
                    if state.running {
                        ...
                        let elapsed_since_start = state.start_time.map(|t| t.elapsed().as_millis() as u64).unwrap_or(0);
...
fn execute_ocr_action_step(step: &crate::model::MacroStep) {
    ...
    fn trigger_vision_move_with_options(
        ...
        loop {
            ...
            let outcome = match run_vision_once_with_options(preset, move_cursor, false, variable_override, None, None) {

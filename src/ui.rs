...
impl CrosshairApp {
    ...
    fn render_mouse_action_group_option(
        ...
        Self::show_instant_hover_tooltip(
            ...
            Self::tr_lang(
                ...
                "ChuÃ¡Â»â„¢t\nMÃ¡Â»Å¸ cÃƒÂ¡c action click, lÃ„Æ’n vÃƒÂ  di chuyÃ¡Â»Æ’n chuÃ¡Â»â„¢t.",
            ),
        );
    }
    ...
    fn capture_button_text(language: UiLanguage, active: bool) -> RichText {
        if active {
            RichText::new(Self::tr_lang(language, "Capturing...", "Ã„Âang bÃ¡ÂºÂ¯t..."))
                .strong()
                ...
                Self::show_preset_card(ui, group.enabled, |ui| {
                    ...
                    for preset in &mut group.presets {
                        ...
                        egui::Grid::new((group.id, preset.id, "preset-summary-row"))
                            ...
                            .spacing([6.0, 4.0])
                            .show(ui, |ui| {
                                ui.label(Self::tr_lang(language, "Trigger", "KÃƒÂ­ch hoÃ¡ÂºÂ¡t"));
                                ui.add_sized(
                                    [148.0, 22.0],
                                    egui::Label::new(
                                        RichText::new(
                                ...
                                egui::ComboBox::from_id_salt((group.id, preset.id, "mouse-trigger"))
                                    .width(96.0)
                                    .selected_text(Self::tr_lang(language, "Mouse", "ChuÃ¡Â»â„¢t"))
                                    .show_ui(ui, |ui| {
                                        for (label, key) in [
                        ...
                        if !preset.collapsed {
                        ui.horizontal(|ui| {
                            ui.label(Self::tr_lang(language, "Trigger Mode", "ChÃ¡ÂºÂ¿ Ã„â€˜Ã¡Â»â„¢ trigger"));
                            egui::ComboBox::from_id_salt((group.id, preset.id, "trigger-mode"))
                                .width(90.0)
                                .selected_text(match (language, preset.trigger_mode) {
                                    (UiLanguage::Vietnamese, MacroTriggerMode::Press) => "NhÃ¡ÂºÂ¥n",
                                    (UiLanguage::Vietnamese, MacroTriggerMode::Hold) => "GiÃ¡Â»Â¯",
                                ...
                                live_sync |= ui
                                    .checkbox(
                                        ...
                                        Self::tr_lang(
                                            ...
                                            "NhÃ¡ÂºÂ¥n trigger lÃ¡ÂºÂ§n nÃ¡Â»Â¯a Ã„â€˜Ã¡Â»Æ’ dÃ¡Â»Â«ng",
                                        ),
                                    )
                                    .on_hover_text(
                                        Self::tr_lang(
                                            ...
                                            "NhÃ¡ÂºÂ¥n trigger lÃ¡ÂºÂ§n nÃ¡Â»Â¯a Ã„â€˜Ã¡Â»Æ’ dÃ¡Â»Â«ng macro ngay lÃ¡ÂºÂ­p tÃ¡Â»Â©c, khÃƒÂ´ng cÃ¡ÂºÂ§n chÃ¡Â»Â step dÃ¡Â»Â«ng.",
                                        ),
                                    )
                                    .changed();
                            } else {
                            ...
                            ui.label(Self::tr_lang(language, "Steps", "BÃ†Â°Ã¡Â»â€ºc"));
                            if Self::sized_button(ui, 68.0, Self::tr_lang(language, "Copy", "Sao chÃƒÂ©p")).clicked() {
                                self.macro_preset_clipboard = Some(preset.clone());
                                self.status = match language {
                                    UiLanguage::Vietnamese => "Ã„ÂÃƒÂ£ sao chÃƒÂ©p preset macro.".to_owned(),
                            ...
                            if Self::sized_button(ui, 86.0, Self::tr_lang(language, "+ Step", "+ BÃ†Â°Ã¡Â»â€ºc")).clicked() {
                                preset.steps.push(MacroStep::default());
                                live_sync = true;
                            }
                            if Self::sized_button(ui, 112.0, Self::tr_lang(language, "Clear Steps", "XÃƒÂ³a bÃ†Â°Ã¡Â»â€ºc")).clicked() {
                                preset.steps.clear();
                                live_sync = true;
                            }
                        });
    ...
    fn render_mouse_panel(&mut self, ui: &mut egui::Ui) {
        ...
        ui.label(self.tr(
            ...
            "Ghi lÃ¡ÂºÂ¡i chuyÃ¡Â»Æ’n Ã„â€˜Ã¡Â»â„¢ng chuÃ¡Â»â„¢t, click vÃƒÂ  wheel vÃƒÂ o thÃ†Â° viÃ¡Â»â€¡n Ã„â€˜Ã¡Â»Æ’ dÃƒÂ¹ng lÃ¡ÂºÂ¡i trong macro.",
        ));
        ui.separator();
        ui.heading(self.tr("Mouse Driver", "Driver chuá»™t"));
        ui.label(self.tr(
            "Use Interception for mouse playback when SendInput does not reach a target app.",
            "DÃ¹ng Interception Ä‘á»ƒ phÃ¡t láº¡i thao tÃ¡c chuá»™t khi SendInput khÃ´ng Ä‘i Ä‘Æ°á»£c vÃ o app má»¥c tiÃªu.",
        ));
        ...
        let lock_download_driver = driver_downloaded || driver_installed;
            ...
            mouse_driver_settings_changed |= ui
                .checkbox(
                    ...
                    Self::tr_lang(
                        ...
                        "Báº­t xuáº¥t chuá»™t báº±ng Interception",
                    ),
                )
                .changed();
            let download_driver_clicked = ui
                .add_enabled(
                    !lock_download_driver,
                    egui::Button::new(Self::tr_lang(
                        self.state.ui_language,
                        if lock_download_driver {
                            "Driver ready"
                        } else {
                            "Download driver"
                        },
        ...
        ui.label(match self.state.ui_language {
            UiLanguage::Vietnamese => {
                ...
                format!(
                    "Backend hiá»‡n táº¡i: {mode}. GÃ³i driver: {package}. Driver há»‡ thá»‘ng: {installed}."
                )
            }
            _ => {
                let mode = if self.state.mouse_use_interception_driver {
        ...
        ui.label(self.tr(
            "Use the keyboard arrow keys to move the mouse. This uses the current mouse backend immediately.",
            "Dung 4 phim mui ten tren ban phim de di chuot. Che do nay dung ngay backend chuot hien tai.",
        ));
        ...
        ui.horizontal_wrapped(|ui| {
            keyboard_arrow_mouse_changed |= ui
                .checkbox(
                    ...
                    Self::tr_lang(
                        self.state.ui_language,
                        "Enable arrow-key mouse movement",
                        "Bat di chuot bang phim mui ten",
                    ),
                )
                .changed();
        ...
        ui.label(match self.state.ui_language {
            UiLanguage::Vietnamese => {
                let backend = if self.state.mouse_use_interception_driver {
                    "Interception"
                } else {
                    "SendInput"
                };
                if self.state.keyboard_arrow_mouse_enabled {
                    format!(
                        "Dang bat. Giu cac phim mui ten de di chuot theo huong tuong ung qua backend {backend}."
                    )
                } else {
                    "Dang tat. Bat len roi giu cac phim mui ten de test ngay trong app/game dich."
                        .to_owned()
                }
            }
            _ => {
                ...
                if self.state.keyboard_arrow_mouse_enabled {
                    format!(
                        "Enabled. Hold the arrow keys to move the mouse through the current {backend} backend."
                    )
                } else {
                    "Disabled. Enable it, then hold the arrow keys to test directly in the target app."
                        .to_owned()
                }
            }
        });

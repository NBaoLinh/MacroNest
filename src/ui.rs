...
use eframe::egui::{
    ...
    Margin, Order, RichText, Sense, Shadow, Slider, Stroke, StrokeKind, TextEdit, TextureHandle,
...
struct ImageSearchCaptureBackground {
    texture: TextureHandle,
    frame: window_list::ScreenCaptureFrame,
}
...
pub struct CrosshairApp {
    ...
    image_search_capture_background: Option<ImageSearchCaptureBackground>,
...
impl CrosshairApp {
    pub fn new(
        ...
        let mut app = Self {
            ...
            image_search_capture_background: None,
    ...
    fn update_image_search_cursor_preview(
        ...
        let (width, height, rgba) = if let Some((width, height, rgba)) =
            self.image_search_background_sample(left, top, sample_size)
        {
            (width, height, rgba)
        } else {
            let capture =
    ...
    fn image_search_background_sample(
        &self,
        left: i32,
        top: i32,
        sample_size: i32,
    ) -> Option<(usize, usize, Vec<u8>)> {
        let background = self.image_search_capture_background.as_ref()?;
        let width = sample_size.max(1) as usize;
        let height = width;
        let mut rgba = vec![0_u8; width * height * 4];
        for y in 0..height {
            let screen_y = top + y as i32;
            let source_y = screen_y - background.frame.screen_y;
            if source_y < 0 || source_y >= background.frame.height as i32 {
                continue;
            }
            for x in 0..width {
                let screen_x = left + x as i32;
                let source_x = screen_x - background.frame.screen_x;
                if source_x < 0 || source_x >= background.frame.width as i32 {
                    continue;
                }
                let source_index =
                    ((source_y as usize * background.frame.width) + source_x as usize) * 4;
                let target_index = ((y * width) + x) * 4;
                if source_index + 3 < background.frame.rgba.len() {
                    rgba[target_index..target_index + 4]
                        .copy_from_slice(&background.frame.rgba[source_index..source_index + 4]);
                }
            }
        }
    ...
    fn render_modal_backdrop(&self, ctx: &egui::Context) {
    ...
    fn capture_hint_text(&self) -> String {
        ...
        self.tr(
            ...
            "Äang á»Ÿ cháº¿ Ä‘á»™ báº¯t phÃ­m. Giá»¯ combo rá»“i tháº£ tay Ä‘á»ƒ lÆ°u. Nháº¥n Esc Ä‘á»ƒ há»§y.",
        )
        .to_owned()
    }
        ...
        Self::show_preset_card(ui, clip.enabled, |ui| {
            ...
            if let Some(total_ms) = *duration_ms {
                ...
                ui.label(format!(
                    ...
                    Self::tr_lang(language, "Slice", "Äoáº¡n hiá»‡n táº¡i"),
                    Self::format_ms(clip.end_ms.saturating_sub(clip.start_ms))
                ));
            }
    ...
    fn begin_capture(&mut self, target: CaptureRequest, status: String) {
        ...
        self.status = if self.capture_request_keeps_open(&target) {
            match self.state.ui_language {
                UiLanguage::Vietnamese => {
                    "Äang báº¯t trigger. Giá»¯ rá»“i nháº£ Ä‘á»ƒ lÆ°u, Esc Ä‘á»ƒ há»§y.".to_owned()
                }
                _ => "Capturing triggers. Hold keys, then release to save. Esc cancels.".to_owned(),
            }
        } else {
            status
        };
    ...
    fn begin_mouse_move_absolute_capture(
        ...
        self.status = Self::tr_lang(
            ...
            "Báº¥m vÃ o báº¥t ká»³ vá»‹ trÃ­ nÃ o trÃªn mÃ n hÃ¬nh Ä‘á»ƒ láº¥y X/Y. Nháº¥n Esc Ä‘á»ƒ há»§y.",
        )
        .to_owned();
    ...
    fn cancel_mouse_move_absolute_capture(&mut self, ctx: &egui::Context) {
        ...
        self.status = Self::tr_lang(
            ...
            "ÄÃ£ há»§y báº¯t tá»a Ä‘á»™ chuá»™t.",
        )
        .to_owned();
        ctx.send_viewport_cmd(egui::ViewportCommand::Visible(true));
        ctx.send_viewport_cmd(egui::ViewportCommand::Focus);
    ...
    fn finish_mouse_move_absolute_capture(
        ...
        let Some(step) = self
            ...
            self.status = Self::tr_lang(
                ...
                "KhÃ´ng tÃ¬m tháº¥y step Ä‘á»ƒ báº¯t tá»a Ä‘á»™ chuá»™t.",
            )
            .to_owned();
            return;
        };
        ...
        self.status = match self.state.ui_language {
            UiLanguage::Vietnamese => {
                format!("ÄÃ£ láº¥y tá»a Ä‘á»™ chuá»™t {}, {}.", screen_x, screen_y)
            }
    ...
    fn pick_point_button_text(language: UiLanguage, active: bool) -> RichText {
        if active {
            RichText::new(Self::tr_lang(language, "Picking...", "Äang chá»n..."))
                .strong()
                .color(Color32::from_rgb(255, 232, 96))
        } else {
    ...
    fn begin_image_search_capture(
        ...
        self.image_search_capture_background = None;
        ...
        self.capture_image_search_background(ctx);
        ctx.send_viewport_cmd(egui::ViewportCommand::Visible(true));
    ...
    fn capture_image_search_background(&mut self, ctx: &egui::Context) {
        let (left, top, width, height) = window_list::virtual_screen_bounds();
        if let Some(frame) = window_list::capture_virtual_screen_region(left, top, width, height) {
            let color_image =
                ColorImage::from_rgba_unmultiplied([frame.width, frame.height], &frame.rgba);
            let texture = ctx.load_texture(
                "image-search-capture-background",
                color_image,
                TextureOptions::LINEAR,
            );
            self.image_search_capture_background =
                Some(ImageSearchCaptureBackground { texture, frame });
        }
    ...
    fn apply_captured_input(&mut self, target: CaptureRequest, captured: CapturedInput) -> bool {
        ...
        match (target, captured) {
            ...
            (CaptureRequest::MacrosMasterHotkey, CapturedInput::Binding(binding)) => {
                ...
                self.persist();
                self.status = match self.state.ui_language {
                    UiLanguage::Vietnamese => "ÄÃ£ gÃ¡n hotkey báº­t/táº¯t macro.".to_owned(),
    ...
    fn render_window_presets_panel(&mut self, ui: &mut egui::Ui) {
        ...
        let language = self.state.ui_language;
        ui.label(
            RichText::new(Self::tr_lang(language, "Focus", "Preset focus cá»­a sá»•")).strong(),
        );
            ...
            Frame::group(ui.style()).show(ui, |ui| {
                ui.horizontal_wrapped(|ui| {
                    ui.label(format!(
                        ...
                        Self::tr_lang(language, "and move", "vÃ  chuyá»ƒn"),
                        Self::tr_lang(
                            language,
                    ...
                    egui::Grid::new((folder_id, "macro-folder-row"))
                        ...
                        .show(ui, |ui| {
                            ...
                            if ui
                                .add_sized(
                                    [70.0, 24.0],
                                    Button::new(Self::tr_lang(language, "Open", "Má»Ÿ")),
                                )
                                .clicked()
                            {
                                open_folder_id = Some(folder_id);
                            ...
                            if ui
                                .add_sized(
                                    [82.0, 24.0],
                                    Button::new(Self::tr_lang(language, "Release", "Nháº£")),
                                )
                                .clicked()
                            {
                                                ...
                                                if step.action == MacroAction::MouseMoveAbsolute {
                                                    ...
                                                    if ui
                                                        ...
                                                        .on_hover_text(Self::tr_lang(
                                                            ...
                                                            "Thu nhá» app rá»“i báº¥m vÃ o báº¥t ká»³ vá»‹ trÃ­ nÃ o trÃªn mÃ n hÃ¬nh Ä‘á»ƒ láº¥y X/Y.",
                                                        ))
                                                    .clicked()
                                                    {
                                                        if capture_active {
    ...
    fn render_image_search_capture_overlay(&mut self, ctx: &egui::Context) -> bool {
        ...
        egui::CentralPanel::default()
            ...
            .show(ctx, |ui| {
                ...
                let painter = ui.painter_at(rect);
                if let Some(background) = self.image_search_capture_background.as_ref() {
                    painter.image(
                    ...
                    painter.rect_filled(rect, 0.0, Color32::from_rgba_premultiplied(0, 0, 0, 24));
                }
            ...
            if s_pressed {
                ...
                match audio::start_preview_from_ms(clip.clone(), preview_start_ms) {
                    Ok(()) => {
                        outcome.status = Some(match language {
                            UiLanguage::Vietnamese => {
                                format!("Äang nghe láº¡i {title} tá»« Ä‘áº§u.")
                            }
                ...
                if previewing {
                    ...
                    match audio::toggle_preview_from_ms(clip.clone(), preview_start_ms) {
                        ...
                        Ok(false) => {
                            outcome.status = Some(match language {
                                UiLanguage::Vietnamese => {
                                    format!("ÄÃ£ dá»«ng nghe thá»­ {title}.")
                                }
                                _ => format!("Stopped {title} preview."),
                            })
                        }
    ...
    fn render_settings_popup(&mut self, ui: &mut egui::Ui) {
        ...
        Frame::group(ui.style()).show(ui, |ui| {
            ...
            ui.horizontal(|ui| {
                ui.label("API Key");
                let key_editor =
                    TextEdit::singleline(&mut self.state.ai_settings.api_key).hint_text("AIza...");
                ai_changed |= ui
                    .add_sized(
                        [(ui.available_width() - 88.0).max(180.0), 24.0],
                        if self.state.ai_settings.show_api_key {
                            key_editor
                        } else {
                    ...
                    .changed();
                if ui
                    .button(if self.state.ai_settings.show_api_key {
                        Self::tr_lang(language, "Hide", "Hide")
                    } else {
                        Self::tr_lang(language, "Show", "Show")
                    })
                    .clicked()
                {
                    self.state.ai_settings.show_api_key = !self.state.ai_settings.show_api_key;
                    ai_changed = true;
                }
            });
        });
